...
- 총 JS/JSX 파일: ${Object.entries(fileAnalysis).filter(([k, v]) => (v.ext === '.js' || v.ext === '.jsx') && v.type === 'file').length}
- 총 CSS 파일: ${Object.entries(fileAnalysis).filter(([k, v]) => v.ext === '.css' && v.type === 'file').length}
- 총 HTML 파일: ${Object.entries(fileAnalysis).filter(([k, v]) => v.ext === '.html' && v.type === 'file').length}
